...
public class BookController {
    ...
    public ResponseEntity<Book> create_Book(@RequestBody Book book) {
        validateBook(book);
        existsBookByEditorialAndEditorial(book);
    ...
    private void validateBook(Book book){
        if(book.getTitle() == null || book.getTitle().isEmpty()){
            throw new ValidationException("El titulo no debe estar vacio");
        }
        if(book.getTitle().length() > 22){
            throw new ValidationException("El titulo no debe exceder los 22 caracteres");
        }
        if(book.getEditorial() == null || book.getEditorial().isEmpty()){
            throw new ValidationException("La editorial del libro no debe estar vacia");
        }
    ...
    private void existsBookByEditorialAndEditorial(Book book){
